import{Q as l}from"./index-DCDSiwCT.js";document.addEventListener("DOMContentLoaded",function(){x(),g()});function g(){document.querySelectorAll('a[href^="#"]').forEach(r=>{r.addEventListener("click",function(e){e.preventDefault();const t=this.getAttribute("href");if(t){const o=document.querySelector(t);o&&o.scrollIntoView({behavior:"smooth",block:"start"})}})})}async function b(r){r.preventDefault();const e=document.getElementById("errorText");e.textContent="",e.classList.add("hidden");const t=document.getElementById("roomName"),o=t==null?void 0:t.value.trim(),m=document.getElementById("userName"),f=m==null?void 0:m.value.trim();if(!o){e.textContent="Room name is required",e.classList.remove("hidden");return}if(!f){e.textContent="User name is required",e.classList.remove("hidden");return}const n=o.trim(),s=f.trim();if(n.length<3){e.textContent="Room name must be at least 3 characters long",e.classList.remove("hidden");return}if(s.length<3){e.textContent="User name must be at least 3 characters long",e.classList.remove("hidden");return}const a=r.target.querySelector('button[type="submit"]');a.innerHTML=`
